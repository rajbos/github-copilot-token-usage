/**
 * Pure statistics helper functions for token usage aggregation.
 * These functions have no VS Code dependencies and can be tested independently
 * with `node --test`.
 */
...
/** UTC date-key boundaries that define the period windows. */
...
/**
 * Merges model usage from `source` into `target`, summing token counts per model.
 */
...
/**
 * Adds editor-typed token usage to `target`, incrementing the session counter by 1.
 */
