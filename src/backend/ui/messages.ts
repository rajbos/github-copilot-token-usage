...
export const ErrorMessages = {
  ...
  connection: (details?: string): string => {
    ...
    return details
      ? `Unable to connect to Azure. ${details} ${suggestion}`
      : `Unable to connect. ${suggestion}`;
  },
  ...
  auth: (details?: string): string => {
    ...
    return details
      ? `Unable to authenticate. ${details} ${suggestion}`
  ...
  sync: (details?: string): string => {
    ...
    return details
      ? `Unable to sync to Azure. ${details} ${suggestion}`
      : `Sync failed. ${suggestion}`;
  },
...
export const ConfirmationMessages = {
  ...
  rotateKey: (): { message: string; detail: string; button: string } => {
    return {
      ...
      detail:
        "You will be prompted to enter the new key. Ensure the new key is valid before proceeding.",
      button: "Replace Key",
    };
  },
  ...
  enableTeamSharing: (): {
    ...
    return {
      ...
      detail:
        "Team members with storage access will see your usage stats. Workspace names will be included if enabled.",
  ...
  disableTeamSharing: (): {
    ...
    return {
      ...
      detail:
        "Workspace and machine IDs will be hashed. Names and user identifiers will be removed from future syncs.",
  ...
  privacyUpgrade: (
    reasons: string[],
  ): { message: string; detail: string; button: string } => {
    const reasonsText =
      reasons.length > 0
        ? reasons.join(" and ")
        : "sharing settings are changing";
    return {
  ...
  /**
   * Per-row data deletion from the Team Dashboard.
   */
  deleteUserDataset: (
    userId: string,
    datasetId: string,
    ...
    return {
      message: `Delete all data for "${userId}" in dataset "${datasetId}"?`,
      detail:
        "This will permanently delete all stored aggregate data for this user and dataset combination from Azure Table Storage. This action cannot be undone.",
      button: "Delete Data",
    };
  },
};
