...
class CopilotTokenTracker implements vscode.Disposable {
  ...
  private parseWorkspaceStorageJsonFile(
    jsonPath: string,
   ...
   */
  private extractWorkspaceIdFromSessionPath(
  ...
  private resolveWorkspaceFolderFromSessionPath(
    sessionFilePath: string,
  ): string | undefined {
    try {
      ...
      const segmentIndex = lowerNormalized.indexOf(
        workspaceSegment.toLowerCase(),
      );
      if (segmentIndex === -1) {
      ...
      const folderPathNormalized = normalized.substring(
        0,
        segmentIndex + workspaceSegment.length,
      );
      ...
      if (fs.existsSync(workspaceJsonPath)) {
        folderFsPath = this.parseWorkspaceStorageJsonFile(workspaceJsonPath, [
          "folder",
          "workspace",
          "configuration",
          "uri",
        ...
        folderFsPath = this.parseWorkspaceStorageJsonFile(metaJsonPath, [
          "folder",
          "uri",
          "workspace",
          "path",
   ...
   */
  private scanWorkspaceCustomizationFiles(
    workspaceFolderPath: string,
  ): CustomizationFileEntry[] {
    const results: CustomizationFileEntry[] = [];
    if (!workspaceFolderPath || !fs.existsSync(workspaceFolderPath)) {
      return results;
    }

    const cfg = customizationPatternsData as any;
    const stalenessDays =
      typeof cfg.stalenessThresholdDays === "number"
        ? cfg.stalenessThresholdDays
        : 90;
    const excludeDirs: string[] = Array.isArray(cfg.excludeDirs)
      ? cfg.excludeDirs
      : [];
        ...
        if (scanMode === "exact") {
          ...
          if (fs.existsSync(absPath)) {
            ...
            results.push({
              path: absPath,
              relativePath: path
                .relative(workspaceFolderPath, absPath)
                .replace(/\\/g, "/"),
          ...
          const baseDir = baseDirPath
            ? path.join(workspaceFolderPath, baseDirPath)
            : workspaceFolderPath;
          ...
          const fullPattern = afterStar.startsWith("/")
            ...
            if (fs.existsSync(candidatePath)) {
              ...
              if (stat.isFile()) {
                ...
                results.push({
                  path: candidatePath,
                  relativePath: path
                    .relative(workspaceFolderPath, candidatePath)
                    .replace(/\\/g, "/"),
                  type: pattern.type || "unknown",
                  icon: pattern.icon || "",
          ...
          const maxDepth =
            typeof pattern.maxDepth === "number" ? pattern.maxDepth : 6;
          ...
          const walk = (dir: string, depth: number) => {
            ...
            for (const child of children) {
              ...
              if (child.isDirectory()) {
                ...
                const rel = path
                  .relative(workspaceFolderPath, path.join(dir, name))
                  .replace(/\\/g, "/");
                if (regex.test(rel)) {
  ...
  private tokenEstimators: { [key: string]: number } =
    tokenEstimatorsData.estimators;
  ...
  private _workspaceIdToFolderCache: Map<string, string | undefined> =
    new Map();

  // Cache mapping workspaceFolderPath -> found customization files (avoid re-scanning)
  private _customizationFilesCache: Map<string, CustomizationFileEntry[]> =
    new Map();
  ...
  private modelPricing: { [key: string]: ModelPricing } =
    modelPricingData.pricing as { [key: string]: ModelPricing };

  // Tool name mapping - loaded from toolNames.json for friendly display names
  private toolNameMap: { [key: string]: string } = toolNamesData as {
    [key: string]: string;
  };
  ...
  private getModeType(
    mode: any,
  ): "ask" | "edit" | "agent" | "plan" | "customAgent" {
    if (!mode || !mode.kind) {
      return "ask";
    }
    ...
    if (mode.kind === "agent") {
      ...
      if (
        ...
        mode.id.includes("plan-agent/Plan.agent.md")
      ) {
        return "plan";
      }
  ...
  private isOpenCodeSessionFile(filePath: string): boolean {
    ...
    return (
      normalized.includes("/opencode/storage/session/") ||
      normalized.includes("/opencode/opencode.db#ses_")
    );
  }
  ...
  private async initSqlJs(): Promise<any> {
    ...
    this._sqlJsModule = await initSqlJs(
    ...
    try {
      ...
      try {
        const result = db.exec(
          "SELECT id, slug, title, time_created, time_updated, project_id, directory FROM session WHERE id = ?",
          [sessionId],
        );
  ...
  private async readOpenCodeDbParts(messageId: string): Promise<any[]> {
    ...
    try {
      ...
      try {
        const result = db.exec(
          "SELECT id, data, time_created FROM part WHERE message_id = ? ORDER BY time_created ASC",
          [messageId],
        );
        if (result.length === 0) {
          return [];
  ...
  private getEditorTypeFromPath(filePath: string): string {
    ...
    if (
      normalizedPath.includes("/code - insiders/") ||
      normalizedPath.includes("/code%20-%20insiders/")
    ) {
      return "VS Code Insiders";
    }
    if (
      normalizedPath.includes("/code - exploration/") ||
      normalizedPath.includes("/code%20-%20exploration/")
    ) {
    ...
    if (
      normalizedPath.includes(".vscode-server/") ||
      normalizedPath.includes(".vscode-remote/")
    ) {
  ...
  private getEditorNameFromRoot(rootPath: string): string {
    ...
    if (lower.includes(".copilot") || lower.includes("copilot")) {
      return "Copilot CLI";
    }
    ...
    if (
      lower.includes("code - insiders") ||
      lower.includes("code-insiders") ||
      lower.includes("insiders")
    ) {
      return "VS Code Insiders";
    }
    if (
      lower.includes("code - exploration") ||
      lower.includes("code%20-%20exploration")
    ) {
      return "VS Code Exploration";
    }
    ...
    if (
      lower.endsWith("code") ||
      lower.includes(path.sep + "code" + path.sep) ||
      lower.includes("/code/")
    ) {
      return "VS Code";
    }
  ...
  private setCachedSessionData(
    ...
    if (this.sessionFileCache.size > 1100) {
      ...
      this.log(
        `Cache size limit reached, removed ${keysToDelete.length} oldest entries. Current size: ${this.sessionFileCache.size}`,
      );
    }
  }
  ...
  private loadCacheFromStorage(): void {
    try {
      ...
      if (storedVersion !== CopilotTokenTracker.CACHE_VERSION) {
        this.log(
          `Cache version mismatch (stored: ${storedVersion}, current: ${CopilotTokenTracker.CACHE_VERSION}) for ${cacheId}. Clearing cache.`,
        );
      ...
      if (cacheData) {
        ...
        this.log(
          `Loaded ${this.sessionFileCache.size} cached session files from storage (${cacheId})`,
        );
      } else {
  ...
  private migrateOldCacheKeys(currentCacheId: string): void {
    try {
      ...
      if (removedCount > 0) {
        this.log(
          `Migrated: removed ${removedCount} old cache keys from globalState`,
        );
      }
    } catch (error) {
  ...
  private async saveCacheToStorage(): Promise<void> {
    ...
    try {
      ...
      this.log(
        `Saved ${this.sessionFileCache.size} cached session files to storage (version ${CopilotTokenTracker.CACHE_VERSION}, ${cacheId})`,
      );
    } catch (error) {
  ...
  public async clearCache(): Promise<void> {
    try {
      ...
      vscode.window.showInformationMessage(
        "Cache cleared successfully. Reloading statistics...",
      );
  ...
  constructor(extensionUri: vscode.Uri, context: vscode.ExtensionContext) {
    ...
    this.statusBarItem.tooltip =
      "Daily and 30-day GitHub Copilot token usage - Click to open details";
  ...
  private scheduleInitialUpdate(): void {
    ...
    if (extensionsExistButInactive) {
      ...
      this.log(
        `⏳ Waiting for Copilot Extension to start (${delaySeconds}s delay)`,
      );
  ...
  public async updateTokenStats(
    silent: boolean = false,
  ): Promise<DetailedStats | undefined> {
    try {
      ...
      tooltip.appendMarkdown(
        `| Tokens :                | ${detailedStats.today.tokens.toLocaleString()} |\n`,
      );
      tooltip.appendMarkdown(
        `| Estimated cost :             | $ ${detailedStats.today.estimatedCost.toFixed(4)} |\n`,
      );
      tooltip.appendMarkdown(
        `| CO₂ estimated :              | ${detailedStats.today.co2.toFixed(2)} grams |\n`,
      );
      tooltip.appendMarkdown(
        `| Water estimated :           | ${detailedStats.today.waterUsage.toFixed(3)} liters |\n`,
      );
      tooltip.appendMarkdown(
        `| Sessions :             | ${detailedStats.today.sessions} |\n`,
      );
      tooltip.appendMarkdown(
        `| Average interactions/session :     | ${detailedStats.today.avgInteractionsPerSession} |\n`,
      );
      tooltip.appendMarkdown(
        `| Average tokens/session :            | ${detailedStats.today.avgTokensPerSession.toLocaleString()} |\n`,
      );
      ...
      tooltip.appendMarkdown(
        `| Tokens :                | ${detailedStats.last30Days.tokens.toLocaleString()} |\n`,
      );
      tooltip.appendMarkdown(
        `| Estimated cost :             | $ ${detailedStats.last30Days.estimatedCost.toFixed(4)} |\n`,
      );
      tooltip.appendMarkdown(
        `| CO₂ estimated :              | ${detailedStats.last30Days.co2.toFixed(2)} grams |\n`,
      );
      tooltip.appendMarkdown(
        `| Water estimated :           | ${detailedStats.last30Days.waterUsage.toFixed(3)} liters |\n`,
      );
      tooltip.appendMarkdown(
        `| Sessions :             | ${detailedStats.last30Days.sessions} |\n`,
      );
      tooltip.appendMarkdown(
        `| Average interactions/session :      | ${detailedStats.last30Days.avgInteractionsPerSession} |\n`,
      );
      tooltip.appendMarkdown(
        `| Average tokens/session :            | ${detailedStats.last30Days.avgTokensPerSession.toLocaleString()} |\n`,
      );
      // Footer
      tooltip.appendMarkdown("\n---\n");
      tooltip.appendMarkdown(
        "*Cost estimates based on actual input/output token ratios.*  \n",
      );
      ...
      if (this.chartPanel) {
        ...
        this.chartPanel.webview.html = this.getChartHtml(
          this.chartPanel.webview,
          dailyStats,
        );
      }
      ...
      if (this.analysisPanel) {
        ...
        this.analysisPanel.webview.html = this.getUsageAnalysisHtml(
          this.analysisPanel.webview,
          analysisStats,
        );
      } else {
      ...
      this.log(
        `Updated stats - Today: ${detailedStats.today.tokens}, Last 30 Days: ${detailedStats.last30Days.tokens}`,
      );
  ...
  private async calculateTokenUsage(): Promise<
    Pick<TokenUsageStats, "todayTokens" | "monthTokens">
  > {
    const now = new Date();
      ...
      for (const sessionFile of sessionFiles) {
        try {
          ...
          if (mtime >= monthStart.getTime()) {
            const tokens = await this.estimateTokensFromSessionCached(
              sessionFile,
              mtime,
              fileSize,
            );
          ...
          this.warn(
            `Error processing session file ${sessionFile}: ${fileError}`,
          );
        }
      }
    } catch (error) {
  ...
  private async calculateDetailedStats(
    ...
    try {
      ...
      if (sessionFiles.length === 0) {
        this.warn(
          "⚠️ No session files found - Have you used GitHub Copilot Chat yet?",
        );
      }
      ...
      for (let i = 0; i < sessionFiles.length; i++) {
        ...
        try {
          ...
          const wasCached =
            ...
            cachedData.mtime === mtime &&
            cachedData.size === fileSize;
          const sessionData = await this.getSessionFileDataCached(
            sessionFile,
            mtime,
            fileSize,
          );
          ...
          if (lastActivity >= last30DaysStart) {
            ...
            if (!last30DaysStats.editorUsage[editorType]) {
              last30DaysStats.editorUsage[editorType] = {
                tokens: 0,
                sessions: 0,
              };
            }
            ...
            for (const [model, usage] of Object.entries(modelUsage)) {
              if (!last30DaysStats.modelUsage[model]) {
                last30DaysStats.modelUsage[model] = {
                  inputTokens: 0,
                  outputTokens: 0,
                };
              }
              last30DaysStats.modelUsage[model].inputTokens +=
                usage.inputTokens;
              last30DaysStats.modelUsage[model].outputTokens +=
                usage.outputTokens;
            }
          }
          ...
          if (lastActivity >= monthStart) {
            ...
            lastActivity >= lastMonthStart &&
            lastActivity <= lastMonthEnd
          ) {
            ...
            if (!lastMonthStats.editorUsage[editorType]) {
              lastMonthStats.editorUsage[editorType] = {
                tokens: 0,
                sessions: 0,
              };
            }
            ...
            for (const [model, usage] of Object.entries(modelUsage)) {
              if (!lastMonthStats.modelUsage[model]) {
                lastMonthStats.modelUsage[model] = {
                  inputTokens: 0,
                  outputTokens: 0,
                };
              }
              lastMonthStats.modelUsage[model].inputTokens += usage.inputTokens;
              lastMonthStats.modelUsage[model].outputTokens +=
                usage.outputTokens;
            }
          } else {
      ...
      this.log(
        `✅ Analysis complete: Today ${todayStats.sessions} sessions, Month ${monthStats.sessions} sessions, Last 30 Days ${last30DaysStats.sessions} sessions, Last Month ${lastMonthStats.sessions} sessions`,
      );
      if (skippedFiles > 0) {
        this.log(
          `⏭️ Skipped ${skippedFiles} session file(s) (empty or no activity in recent months)`,
        );
      }
      const totalCacheAccesses = cacheHits + cacheMisses;
      this.log(
        `💾 Cache performance: ${cacheHits} hits, ${cacheMisses} misses (${totalCacheAccesses > 0 ? ((cacheHits / totalCacheAccesses) * 100).toFixed(1) : 0}% hit rate)`,
      );
    } catch (error) {
    ...
    const lastMonthWater =
      (lastMonthStats.tokens / 1000) * this.waterUsagePer1kTokens;
    const last30DaysWater =
      (last30DaysStats.tokens / 1000) * this.waterUsagePer1kTokens;
    ...
    const lastMonthCost = this.calculateEstimatedCost(
      lastMonthStats.modelUsage,
    );
    const last30DaysCost = this.calculateEstimatedCost(
      last30DaysStats.modelUsage,
    );
    ...
    const result: DetailedStats = {
      today: {
        ...
        avgInteractionsPerSession:
          todayStats.sessions > 0
            ? Math.round(todayStats.interactions / todayStats.sessions)
            : 0,
        avgTokensPerSession:
          todayStats.sessions > 0
            ? Math.round(todayStats.tokens / todayStats.sessions)
            : 0,
      ...
      month: {
        ...
        avgInteractionsPerSession:
          monthStats.sessions > 0
            ? Math.round(monthStats.interactions / monthStats.sessions)
            : 0,
        avgTokensPerSession:
          monthStats.sessions > 0
            ? Math.round(monthStats.tokens / monthStats.sessions)
            : 0,
      ...
      lastMonth: {
        ...
        avgInteractionsPerSession:
          lastMonthStats.sessions > 0
            ? Math.round(lastMonthStats.interactions / lastMonthStats.sessions)
            : 0,
        avgTokensPerSession:
          lastMonthStats.sessions > 0
            ? Math.round(lastMonthStats.tokens / lastMonthStats.sessions)
            : 0,
      ...
      last30Days: {
        ...
        avgInteractionsPerSession:
          last30DaysStats.sessions > 0
            ? Math.round(
                last30DaysStats.interactions / last30DaysStats.sessions,
              )
            : 0,
        avgTokensPerSession:
          last30DaysStats.sessions > 0
            ? Math.round(last30DaysStats.tokens / last30DaysStats.sessions)
            : 0,
  ...
  private async calculateDailyStats(): Promise<DailyTokenStats[]> {
    ...
    try {
      const sessionFiles = await this.getCopilotSessionFiles();
      this.log(
        `📈 Preparing chart data from ${sessionFiles.length} session file(s)...`,
      );
        ...
        try {
          ...
          if (mtime >= thirtyDaysAgo.getTime()) {
            ...
            if (!dailyStats.repositoryUsage[repository]) {
              dailyStats.repositoryUsage[repository] = {
                tokens: 0,
                sessions: 0,
              };
            }
          ...
          this.warn(
            `Error processing session file ${sessionFile} for daily stats: ${fileError}`,
          );
        }
      }
    } catch (error) {
    ...
    let dailyStatsArray = Array.from(dailyStatsMap.values()).sort((a, b) =>
    ...
    dailyStatsArray = Array.from(dailyStatsMap.values()).sort((a, b) =>
      a.date.localeCompare(b.date),
    );
   ...
   */
  private async calculateUsageAnalysisStats(
    useCache = true,
  ): Promise<UsageAnalysisStats> {
    ...
    try {
      const sessionFiles = await this.getCopilotSessionFiles();
      this.log(
        `🔍 [Usage Analysis] Processing ${sessionFiles.length} session files`,
      );

      let processed = 0;
      const progressInterval = Math.max(
        1,
        Math.floor(sessionFiles.length / 20),
      ); // Log every 5%
      ...
      for (const sessionFile of sessionFiles) {
        try {
          ...
          if (mtime >= last30DaysStart.getTime()) {
            ...
            if (interactions === 0) {
              ...
              if (processed % progressInterval === 0) {
                this.log(
                  `🔍 [Usage Analysis] Progress: ${processed}/${sessionFiles.length} files (${Math.round((processed / sessionFiles.length) * 100)}%)`,
                );
              }
              continue;
            }
            ...
            const workspaceId =
              this.extractWorkspaceIdFromSessionPath(sessionFile);
            try {
              const workspaceFolder =
                this.resolveWorkspaceFolderFromSessionPath(sessionFile);
              if (workspaceFolder) {
                const norm = path.normalize(workspaceFolder);
                workspaceSessionCounts.set(
                  norm,
                  (workspaceSessionCounts.get(norm) || 0) + 1,
                );
                workspaceInteractionCounts.set(
                  norm,
                  (workspaceInteractionCounts.get(norm) || 0) + interactions,
                );
                ...
                unresolvedWorkspaceInteractionCounts.set(
                  workspaceId,
                  (unresolvedWorkspaceInteractionCounts.get(workspaceId) || 0) +
                    interactions,
                );
              }
            } catch (e) {
          ...
          this.warn(
            `Error processing session file ${sessionFile} for usage analysis: ${fileError}`,
          );
          processed++;
        }
      }
      ...
      try {
        ...
        const customizationTypes = Array.from(uniqueTypes.entries()).map(
          ([id, v]) => ({ id, icon: v.icon, label: v.label }),
        );
        ...
        for (const [folderPath, sessionCount] of workspaceSessionCounts) {
          ...
          const hasNoCustomizationFiles = customizationTypes.every(
            (t) => typeStatuses[t.id] === "❌",
          );
          if (hasNoCustomizationFiles) {
        ...
        for (const workspaceId of unresolvedWorkspaceIds) {
          ...
          const displayId =
            workspaceId.length > CopilotTokenTracker.WORKSPACE_ID_DISPLAY_LENGTH
    ...
    this.log(
      `🔍 [Usage Analysis] Cache stats: ${this._cacheHits} hits, ${this._cacheMisses} misses`,
    );
  ...
  private mergeUsageAnalysis(
    ...
    for (const [tool, count] of Object.entries(analysis.toolCalls.byTool)) {
      period.toolCalls.byTool[tool] =
        (period.toolCalls.byTool[tool] || 0) + count;
    }
    ...
    period.contextReferences.implicitSelection +=
      analysis.contextReferences.implicitSelection || 0;
    ...
    period.contextReferences.terminalLastCommand +=
      analysis.contextReferences.terminalLastCommand || 0;
    period.contextReferences.terminalSelection +=
      analysis.contextReferences.terminalSelection || 0;
    period.contextReferences.clipboard +=
      analysis.contextReferences.clipboard || 0;
    period.contextReferences.changes += analysis.contextReferences.changes || 0;
    period.contextReferences.outputPanel +=
      analysis.contextReferences.outputPanel || 0;
    period.contextReferences.problemsPanel +=
      analysis.contextReferences.problemsPanel || 0;

    // Merge contentReferences counts
    period.contextReferences.copilotInstructions +=
      analysis.contextReferences.copilotInstructions || 0;
    period.contextReferences.agentsMd +=
      analysis.contextReferences.agentsMd || 0;

    // Merge byKind tracking
    for (const [kind, count] of Object.entries(
      analysis.contextReferences.byKind || {},
    )) {
      period.contextReferences.byKind[kind] =
        (period.contextReferences.byKind[kind] || 0) + count;
    }

    // Merge byPath tracking
    for (const [path, count] of Object.entries(
      analysis.contextReferences.byPath || {},
    )) {
      period.contextReferences.byPath[path] =
        (period.contextReferences.byPath[path] || 0) + count;
    }

    // Merge MCP tools
    period.mcpTools.total += analysis.mcpTools.total;
    for (const [server, count] of Object.entries(analysis.mcpTools.byServer)) {
      period.mcpTools.byServer[server] =
        (period.mcpTools.byServer[server] || 0) + count;
    }
    for (const [tool, count] of Object.entries(analysis.mcpTools.byTool)) {
      period.mcpTools.byTool[tool] =
        (period.mcpTools.byTool[tool] || 0) + count;
    }
    ...
    if (analysis.modelSwitching.modelCount > 0) {
      ...
      period.modelSwitching.modelsPerSession.push(
        analysis.modelSwitching.modelCount,
      );
      ...
      period.modelSwitching.standardRequests +=
        analysis.modelSwitching.standardRequests || 0;
      period.modelSwitching.premiumRequests +=
        analysis.modelSwitching.premiumRequests || 0;
      period.modelSwitching.unknownRequests +=
        analysis.modelSwitching.unknownRequests || 0;
      period.modelSwitching.totalRequests +=
      ...
      if (period.modelSwitching.modelsPerSession.length > 0) {
        ...
        period.modelSwitching.averageModelsPerSession =
        ...
        period.modelSwitching.switchingFrequency =
          (counts.filter((c) => c > 1).length / counts.length) * 100;
      }
    }
    ...
    if (analysis.editScope) {
      ...
      // Recalculate average
      const editSessions =
        period.editScope.singleFileEdits + period.editScope.multiFileEdits;
      period.editScope.avgFilesPerSession =
        editSessions > 0 ? period.editScope.totalEditedFiles / editSessions : 0;
    }
    ...
    if (analysis.applyUsage) {
      ...
      period.applyUsage.applyRate =
        period.applyUsage.totalCodeBlocks > 0
          ? (period.applyUsage.totalApplies /
              period.applyUsage.totalCodeBlocks) *
            100
          : 0;
    }

    if (analysis.sessionDuration) {
      period.sessionDuration.totalDurationMs +=
        analysis.sessionDuration.totalDurationMs;
      // Calculate avgDurationMs as total / sessionCount
      const sessionCount = period.sessions;
      if (sessionCount > 0) {
        period.sessionDuration.avgDurationMs =
          period.sessionDuration.totalDurationMs / sessionCount;

        // For other timing metrics, use weighted averaging (approximation across per-session averages)
        const prevAvgFirstProgress =
          period.sessionDuration.avgFirstProgressMs * (sessionCount - 1);
        period.sessionDuration.avgFirstProgressMs =
          (prevAvgFirstProgress + analysis.sessionDuration.avgFirstProgressMs) /
          sessionCount;

        const prevAvgTotalElapsed =
          period.sessionDuration.avgTotalElapsedMs * (sessionCount - 1);
        period.sessionDuration.avgTotalElapsedMs =
          (prevAvgTotalElapsed + analysis.sessionDuration.avgTotalElapsedMs) /
          sessionCount;

        const prevAvgWaitTime =
          period.sessionDuration.avgWaitTimeMs * (sessionCount - 1);
        period.sessionDuration.avgWaitTimeMs =
          (prevAvgWaitTime + analysis.sessionDuration.avgWaitTimeMs) /
          sessionCount;
      }
    }

    if (analysis.conversationPatterns) {
      period.conversationPatterns.multiTurnSessions +=
        analysis.conversationPatterns.multiTurnSessions;
      period.conversationPatterns.singleTurnSessions +=
        analysis.conversationPatterns.singleTurnSessions;
      ...
      if (totalSessions > 0) {
        ...
        const prevTotalTurns =
          period.conversationPatterns.avgTurnsPerSession * (totalSessions - 1);
        // Add current session's turn count (which is stored in avgTurnsPerSession for single session)
        const newTotalTurns =
          prevTotalTurns + analysis.conversationPatterns.avgTurnsPerSession;
        // Calculate true average
        period.conversationPatterns.avgTurnsPerSession =
          newTotalTurns / totalSessions;
      }
    }
  ...
  private async countInteractionsInSession(
    sessionFile: string,
  ): Promise<number> {
    try {
      ...
      const isJsonlContent =
        sessionFile.endsWith(".jsonl") || this.isJsonlContent(fileContent);
      if (isJsonlContent) {
  ...
  private async getModelUsageFromSession(
    sessionFile: string,
  ): Promise<ModelUsage> {
      ...
      if (isJsonlContent) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (event.kind === 0) {
              const modelId =
                event.v?.selectedModel?.identifier ||
            ...
            if (!isDeltaBased) {
              ...
              if (event.type === "user.message" && event.data?.content) {
                modelUsage[model].inputTokens += this.estimateTokensFromText(
                  event.data.content,
                  model,
                );
              } else if (
                ...
                modelUsage[model].outputTokens += this.estimateTokensFromText(
                  event.data.content,
                  model,
                );
        ...
        if (
          ...
          Array.isArray(sessionState.requests)
        ) {
          for (const request of sessionState.requests) {
            ...
            if (request.modelId) {
              ...
              requestModel = request.result.metadata.modelId.replace(
                /^copilot\//,
                "",
              );
        ...
        for (const request of sessionContent.requests) {
          ...
          if (request.result?.usage) {
            ...
            if (request.response && Array.isArray(request.response)) {
              for (const responseItem of request.response) {
                if (responseItem.value) {
                  const tokens = this.estimateTokensFromText(
                    responseItem.value,
                    model,
                  );
   ...
   */
  private async analyzeSessionUsage(
    sessionFile: string,
  ): Promise<SessionUsageAnalysis> {
    ...
    try {
              ...
              for (const part of parts) {
                if (part.type === "tool" && part.tool) {
                  ...
                  analysis.toolCalls.byTool[toolName] =
                    (analysis.toolCalls.byTool[toolName] || 0) + 1;
                }
              }
            }
          }
      ...
      if (isJsonlContent) {
        ...
        if (isDeltaBased) {
          ...
          if (
            sessionState.inputState?.selections &&
            Array.isArray(sessionState.inputState.selections)
          ) {
              ...
              for (const responseItem of request.response) {
                if (
                  ...
                  const toolName =
                    ...
                    responseItem.toolSpecificData?.kind ||
                    "unknown";
                  ...
                  if (this.isMcpTool(toolName)) {
                    ...
                    analysis.mcpTools.byServer[serverName] =
                      (analysis.mcpTools.byServer[serverName] || 0) + 1;
                    const normalizedTool = this.normalizeMcpToolName(toolName);
                    analysis.mcpTools.byTool[normalizedTool] =
                      (analysis.mcpTools.byTool[normalizedTool] || 0) + 1;
                  } else {
                    analysis.toolCalls.total++;
                    analysis.toolCalls.byTool[toolName] =
                      (analysis.toolCalls.byTool[toolName] || 0) + 1;
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (event.kind === 0 && event.v?.inputState?.mode) {
              ...
              if (
                event.v?.inputState?.selections &&
                Array.isArray(event.v.inputState.selections)
              ) {
            ...
            if (
              event.kind === 1 &&
              event.k?.includes("contentReferences") &&
              Array.isArray(event.v)
            ) {
              this.analyzeContentReferences(
                event.v,
            ...
            if (event.type === "mcp.tool.call" || event.data?.mcpServer) {
              ...
              analysis.mcpTools.byTool[normalizedMcpTool] =
                (analysis.mcpTools.byTool[normalizedMcpTool] || 0) + 1;
            }
          } catch (e) {
            // Skip malformed lines
          }
        ...
        for (const request of sessionContent.requests) {
          ...
          if (request.response && Array.isArray(request.response)) {
            for (const responseItem of request.response) {
              ...
              if (
                responseItem.kind === "mcpServersStarting" &&
                responseItem.didStartServerIds
              ) {
                for (const serverId of responseItem.didStartServerIds) {
                  analysis.mcpTools.total++;
                  analysis.mcpTools.byServer[serverId] =
                    (analysis.mcpTools.byServer[serverId] || 0) + 1;
                }
              }

              // Detect inline references in response items
              if (
                responseItem.kind === "inlineReference" &&
                responseItem.inlineReference
              ) {
   ...
   */
  private async calculateModelSwitching(
    sessionFile: string,
    analysis: SessionUsageAnalysis,
  ): Promise<void> {
    try {
      ...
      analysis.modelSwitching.tiers = {
        standard: standardModels,
        premium: premiumModels,
        unknown: unknownModels,
      };
      analysis.modelSwitching.hasMixedTiers =
      ...
      if (!isJsonlContent) {
        ...
        if (sessionContent.requests && Array.isArray(sessionContent.requests)) {
          ...
          analysis.modelSwitching.totalRequests =
            tierCounts.standard + tierCounts.premium + tierCounts.unknown;
        }
      } else {
      ...
      this.warn(
        `Error calculating model switching for ${sessionFile}: ${error}`,
      );
    }
  }
  ...
  private normalizeMcpToolName(toolName: string): string {
    if (toolName.startsWith("mcp_github_github_")) {
      return (
        "mcp_io_github_git_" + toolName.substring("mcp_github_github_".length)
      );
    }
    if (toolName.startsWith("mcp.github.github.")) {
      return (
        "mcp.io.github.git." + toolName.substring("mcp.github.github.".length)
      );
    }
    return toolName;
  }
  ...
  private extractMcpServerName(toolName: string): string {
    ...
    if (
      toolName.startsWith("mcp_io_github_git_") ||
      toolName.startsWith("mcp.io.github.git.")
    ) {
      return "GitHub MCP (Local)";
    }
    if (
      toolName.startsWith("mcp_github_github_") ||
      toolName.startsWith("mcp.github.github.")
    ) {
    ...
    try {
      ...
      if (isJsonlContent) {
        ...
        if (isDeltaBased) {
          ...
          if (sessionState.lastMessageDate) {
            timestamps.push(sessionState.lastMessageDate);
          }
        ...
        // Extract timestamps
        if (sessionContent.creationDate) {
          timestamps.push(sessionContent.creationDate);
        }
        if (sessionContent.lastMessageDate) {
          timestamps.push(sessionContent.lastMessageDate);
        }
      ...
      analysis.editScope = {
        ...
        avgFilesPerSession:
          editSessionCount > 0 ? editedFiles.size / editSessionCount : 0,
      };
      ...
      analysis.applyUsage = {
        ...
        totalCodeBlocks,
        applyRate:
          totalCodeBlocks > 0 ? (totalApplies / totalCodeBlocks) * 100 : 0,
      };
      ...
      const totalDurationMs =
        timestamps.length >= 2
          ...
          : 0;
      const avgFirstProgressMs =
        timingsData.length > 0
          ? timingsData.reduce((sum, t) => sum + (t.firstProgress || 0), 0) /
            timingsData.length
          : 0;
      const avgTotalElapsedMs =
        timingsData.length > 0
          ? timingsData.reduce((sum, t) => sum + (t.totalElapsed || 0), 0) /
            timingsData.length
          : 0;
      const avgWaitTimeMs =
        waitTimes.length > 0
      ...
      this.warn(
        `Error tracking enhanced metrics from ${sessionFile}: ${error}`,
      );
    }
  }
   ...
   */
  private analyzeContextReferences(
    text: string,
    refs: ContextReferenceUsage,
  ): void {
    ...
    for (const contentRef of contentReferences) {
      ...
      if (reference) {
        ...
        if (typeof fsPath === "string") {
          ...
          const pathKey =
            fsPath.length > 100
              ? "..." + fsPath.substring(fsPath.length - 97)
              : fsPath;
   ...
   */
  private async extractRepositoryFromContentReferences(
    contentReferences: any[],
      ...
      if (this.isOpenCodeSessionFile(sessionFile)) {
        ...
        if (session) {
          ...
          if (session.time?.created) {
            timestamps.push(session.time.created);
          }
          if (session.time?.updated) {
            timestamps.push(session.time.updated);
          }
        }
        ...
        for (const msg of messages) {
          if (msg.time?.created) {
            timestamps.push(msg.time.created);
          }
          if (msg.time?.completed) {
            timestamps.push(msg.time.completed);
          }
        }
      ...
      if (isJsonlContent) {
        ...
        try {
          ...
          if (parsed.creationDate) {
            timestamps.push(parsed.creationDate);
          }
          ...
          if (parsed.requests && Array.isArray(parsed.requests)) {
            for (const request of parsed.requests) {
              if (
                ...
                const ts =
                  request.timestamp || request.ts || request.result?.timestamp;
                timestamps.push(new Date(ts).getTime());
              }
            }
          }
  ...
  private async getSessionFileDataCached(
    sessionFilePath: string,
    mtime: number,
    fileSize: number,
  ...
  private async estimateTokensFromSessionCached(
    sessionFilePath: string,
    mtime: number,
    fileSize: number,
  ): Promise<number> {
  ...
  private async countInteractionsInSessionCached(
    sessionFile: string,
    mtime: number,
    fileSize: number,
  ): Promise<number> {
  ...
  private async getModelUsageFromSessionCached(
    sessionFile: string,
    mtime: number,
    fileSize: number,
  ): Promise<ModelUsage> {
  ...
  private async getUsageAnalysisFromSessionCached(
    sessionFile: string,
    mtime: number,
    fileSize: number,
  ): Promise<SessionUsageAnalysis> {
   ...
   */
  private enrichDetailsWithEditorInfo(
    sessionFile: string,
   ...
   */
  private async getSessionFileDetailsFromCache(
    sessionFile: string,
    stat: fs.Stats,
  ): Promise<SessionFileDetails | undefined> {
    ...
    if (
      !cached.usageAnalysis?.contextReferences ||
      typeof cached.interactions !== "number" ||
      cached.interactions < 0
    ) {
      return undefined;
    }
   ...
   */
  private async getSessionFileDetails(
    sessionFile: string,
  ): Promise<SessionFileDetails> {
    const stat = await this.statSessionFile(sessionFile);

    // Try to get details from cache first
    const cachedDetails = await this.getSessionFileDetailsFromCache(
      sessionFile,
      stat,
    );
    if (cachedDetails) {
      ...
      if (
        cachedDetails.repository === undefined &&
    ...
    try {
      ...
      if (this.isOpenCodeSessionFile(sessionFile)) {
        ...
        details.interactions =
          await this.countOpenCodeInteractions(sessionFile);
        const timestamps: number[] = [];
        if (session?.time?.created) {
          timestamps.push(session.time.created);
        }
        ...
        if (timestamps.length > 0) {
          ...
          details.lastInteraction = new Date(
            timestamps[timestamps.length - 1],
          ).toISOString();
        }
      ...
      if (isJsonlContent) {
        ...
        if (isDeltaBased) {
          ...
          for (const request of requests) {
            ...
            if (
              request.contentReferences &&
              Array.isArray(request.contentReferences)
            ) {
          ...
          if (allContentReferences.length > 0) {
            details.repository =
              await this.extractRepositoryFromContentReferences(
          ...
          try {
            ...
            if (event.type === "user.message") {
              ...
              if (event.data?.content) {
                this.analyzeContextReferences(
                  event.data.content,
                  details.contextReferences,
                );
              }
            }
          } catch {
      ...
      const hasRequests =
        sessionContent.requests && Array.isArray(sessionContent.requests);
      ...
      if (hasRequests) {
        ...
        for (const request of sessionContent.requests) {
          ...
          if (request.variableData) {
            const varDataStr = JSON.stringify(
              request.variableData,
            ).toLowerCase();
            if (varDataStr.includes("workspace")) {
              details.contextReferences.workspace++;
            }
            if (varDataStr.includes("terminal")) {
              details.contextReferences.terminal++;
            }
            if (varDataStr.includes("vscode")) {
              details.contextReferences.vscode++;
            }
          }
        }
      ...
      this.warn(
        `Error analyzing session file details for ${sessionFile}: ${error}`,
      );
    }
  ...
  private detectEditorSource(filePath: string): string {
    ...
    if (lowerPath.includes("/.copilot/session-state/")) {
      return "Copilot CLI";
    }
    if (this.isOpenCodeSessionFile(filePath)) {
      return "OpenCode";
    ...
    if (
      lowerPath.includes("code - insiders") ||
      lowerPath.includes("code-insiders")
    ) {
      return "VS Code Insiders";
    }
    if (lowerPath.includes("vscodium")) {
      return "VSCodium";
    }
    if (lowerPath.includes("windsurf")) {
      return "Windsurf";
    }
    if (lowerPath.includes("code")) {
      return "VS Code";
    }
    return "Unknown";
  }
   ...
   */
  private async getSessionLogData(
    sessionFile: string,
    ...
    try {
      ...
      if (this.isOpenCodeSessionFile(sessionFile)) {
        ...
        if (messages.length > 0) {
          ...
          for (let i = 0; i < messages.length; i++) {
            ...
            const turnAssistantMsgs = messages.filter(
              (m, idx) =>
                idx > i && m.role === "assistant" && m.parentID === msg.id,
            );
            ...
            for (const assistantMsg of turnAssistantMsgs) {
              ...
              if (typeof assistantMsg.tokens?.total === "number") {
                turnCumulativeTotal = Math.max(
                  turnCumulativeTotal,
                  assistantMsg.tokens.total,
                );
              }
              const assistantParts = await this.getOpenCodePartsForMessage(
                assistantMsg.id,
              );
            ...
            const turnOutputAndThinking = turnAssistantMsgs.reduce(
              (sum, m) =>
                sum + (m.tokens?.output || 0) + (m.tokens?.reasoning || 0),
              0,
            );
            const turnInputTokens = Math.max(
              0,
              turnTokens - turnOutputAndThinking,
            );
            ...
            turns.push({
              ...
              timestamp: msg.time?.created
                ...
                : null,
              mode:
                msg.agent === "build" || msg.agent === "agent"
                  ? "agent"
                  : msg.agent === "ask"
                    ? "ask"
                    : "agent",
      ...
      if (isJsonlContent) {
        ...
        if (isDeltaBased) {
          ...
          let sessionMode: "ask" | "edit" | "agent" | "plan" | "customAgent" =
            "ask";
          ...
          for (let i = 0; i < requests.length; i++) {
            ...
            const { responseText, thinkingText, toolCalls, mcpTools } =
              this.extractResponseData(request.response || []);
            ...
            if (request.result?.usage) {
              ...
              actualUsage = {
                ...
                promptTokens:
                  typeof u.promptTokens === "number" ? u.promptTokens : 0,
                promptTokenDetails: Array.isArray(u.promptTokenDetails)
                  ? u.promptTokenDetails
                  : undefined,
                details:
                  typeof request.result.details === "string"
            ...
            const turn: ChatTurn = {
              ...
              mcpTools,
              inputTokensEstimate: this.estimateTokensFromText(
                userMessage,
                requestModel,
              ),
              outputTokensEstimate: this.estimateTokensFromText(
                responseText,
                requestModel,
              ),
              thinkingTokensEstimate: this.estimateTokensFromText(
                thinkingText,
                requestModel,
              ),
              actualUsage,
          ...
          for (const line of lines) {
            try {
              ...
              if (
                ...
                lastTurn.outputTokensEstimate = this.estimateTokensFromText(
                  lastTurn.assistantResponse,
                  lastTurn.model || "gpt-4o",
                );
              }
              ...
              if (
                (event.type === "mcp.tool.call" || event.data?.mcpServer) &&
                turns.length > 0
              ) {
                const lastTurn = turns[turns.length - 1];
                const serverName = event.data?.mcpServer || "unknown";
    ...
    try {
      ...
      usageAnalysis = await this.getUsageAnalysisFromSessionCached(
        sessionFile,
        mtimeMs,
        details.size,
      );
      ...
      this.warn(
        `Error loading usage analysis for ${sessionFile}: ${usageError}`,
      );
    }
    ...
    for (const item of response) {
      ...
      if (
        ...
        const toolName =
          item.toolId ||
          item.toolName ||
          item.invocationMessage?.toolName ||
          item.toolSpecificData?.kind ||
          "unknown";
  ...
  private calculateEstimatedCost(modelUsage: ModelUsage): number {
    ...
    for (const [model, usage] of Object.entries(modelUsage)) {
      ...
      if (pricing) {
        ...
        const inputCost =
          (usage.inputTokens / 1_000_000) * fallbackPricing.inputCostPerMillion;
        const outputCost =
          (usage.outputTokens / 1_000_000) *
          fallbackPricing.outputCostPerMillion;

        totalCost += inputCost + outputCost;

        this.log(
          `No pricing data for model '${model}', using fallback pricing (gpt-4o-mini)`,
        );
      }
    }
  ...
  private checkCopilotExtension(): void {
    ...
    if (!copilotExtension && !copilotChatExtension) {
      ...
      const copilotStatus = copilotExtension
        ? copilotExtension.isActive
          ...
          : "⏳ Loading"
        : "❌ Not found";
      const chatStatus = copilotChatExtension
        ? copilotChatExtension.isActive
          ? "✅ Active"
          : "⏳ Loading"
        : "❌ Not found";
    ...
    if (
      isCodespaces &&
      (!copilotExtension?.isActive || !copilotChatExtension?.isActive)
    ) {
  ...
  private getVSCodeUserPaths(): string[] {
    ...
    if (platform === "win32") {
      ...
      for (const variant of vscodeVariants) {
        paths.push(
          path.join(homedir, "Library", "Application Support", variant, "User"),
        );
      }
    } else {
   ...
   */
  private getDiagnosticCandidatePaths(): {
    path: string;
    exists: boolean;
    source: string;
  }[] {
    ...
    try {
      copilotCliExists = fs.existsSync(copilotCliPath);
    } catch {
      /* ignore */
    }
    candidates.push({
      path: copilotCliPath,
      exists: copilotCliExists,
      source: "Copilot CLI",
    });
    ...
    try {
      openCodeJsonExists = fs.existsSync(openCodeSessionDir);
    } catch {
      /* ignore */
    }
    candidates.push({
      path: openCodeSessionDir,
    ...
    try {
      openCodeDbExists = fs.existsSync(openCodeDbPath);
    } catch {
      /* ignore */
    }
    candidates.push({
  ...
  private async getCopilotSessionFiles(): Promise<string[]> {
    ...
    const now = Date.now();
    if (
      this._sessionFilesCache &&
      now - this._sessionFilesCacheTime <
        CopilotTokenTracker.SESSION_FILES_CACHE_TTL
    ) {
      this.log(
        `💨 Using cached session files list (${this._sessionFilesCache.length} files, cached ${Math.round((now - this._sessionFilesCacheTime) / 1000)}s ago)`,
      );
    ...
    const allVSCodePaths = this.getVSCodeUserPaths();
    this.log(
      `📂 Considering ${allVSCodePaths.length} candidate VS Code paths:`,
    );
    ...
    for (let i = 0; i < allVSCodePaths.length; i++) {
      ...
      if ((i + 1) % 5 === 0 || i === allVSCodePaths.length - 1) {
        this.log(
          `📂 Reading local folders [${i + 1}/${allVSCodePaths.length}]`,
        );
      }
    }

    this.log(
      `✅ Found ${foundPaths.length} of ${allVSCodePaths.length} VS Code paths exist on disk:`,
    );
    ...
    try {
      ...
      for (let i = 0; i < foundPaths.length; i++) {
                  ...
                  if (fs.existsSync(chatSessionsPath)) {
                    try {
                      ...
                      if (sessionFiles2.length > 0) {
                        this.log(
                          `📄 Found ${sessionFiles2.length} session files in ${pathName}/workspaceStorage/${workspaceDir}`,
                        );
                        sessionFiles.push(...sessionFiles2);
                      }
                    } catch (readError) {
                      this.warn(
                        `Could not read chat sessions in ${chatSessionsPath}: ${readError}`,
                      );
                    }
                  }
                } catch (checkError) {
                  this.warn(
                    `Could not check chat sessions path ${chatSessionsPath}: ${checkError}`,
                  );
                }
              }
            } catch (readError) {
              this.warn(
                `Could not read workspace storage in ${workspaceStoragePath}: ${readError}`,
              );
            }
          }
        } catch (checkError) {
          this.warn(
            `Could not check workspace storage path ${workspaceStoragePath}: ${checkError}`,
          );
        }
        ...
        try {
          if (fs.existsSync(globalStoragePath)) {
            try {
              const globalSessionFiles = fs
                .readdirSync(globalStoragePath)
                .filter(
              ...
              if (globalSessionFiles.length > 0) {
                this.log(
                  `📄 Found ${globalSessionFiles.length} session files in ${pathName}/globalStorage/emptyWindowChatSessions`,
                );
                sessionFiles.push(...globalSessionFiles);
              }
            } catch (readError) {
              this.warn(
                `Could not read global storage in ${globalStoragePath}: ${readError}`,
              );
            }
          }
        } catch (checkError) {
          this.warn(
            `Could not check global storage path ${globalStoragePath}: ${checkError}`,
          );
        }

        // GitHub Copilot Chat extension global storage
        const copilotChatGlobalPath = path.join(
          codeUserPath,
          "globalStorage",
          "github.copilot-chat",
        );
        try {
          if (fs.existsSync(copilotChatGlobalPath)) {
            this.log(
              `📄 Scanning ${pathName}/globalStorage/github.copilot-chat`,
            );
            this.scanDirectoryForSessionFiles(
              copilotChatGlobalPath,
              sessionFiles,
            );
          }
        } catch (checkError) {
          this.warn(
            `Could not check Copilot Chat global storage path ${copilotChatGlobalPath}: ${checkError}`,
          );
        }
      }

      // Check for Copilot CLI session-state directory (new location for agent mode sessions)
      const copilotCliSessionPath = path.join(
        os.homedir(),
        ".copilot",
        "session-state",
      );
      this.log(
        `📁 Checking Copilot CLI path: ${copilotCliSessionPath} (exists: ${fs.existsSync(copilotCliSessionPath)})`,
      );
      try {
        if (fs.existsSync(copilotCliSessionPath)) {
          try {
            const entries = fs.readdirSync(copilotCliSessionPath, {
              withFileTypes: true,
            });
            ...
            if (cliSessionFiles.length > 0) {
              this.log(
                `📄 Found ${cliSessionFiles.length} session files in Copilot CLI directory`,
              );
            ...
            if (subDirSessionCount > 0) {
              this.log(
                `📄 Found ${subDirSessionCount} session files in Copilot CLI subdirectories`,
              );
            }
          } catch (readError) {
            this.warn(
              `Could not read Copilot CLI session path in ${copilotCliSessionPath}: ${readError}`,
            );
          }
        }
      } catch (checkError) {
        this.warn(
          `Could not check Copilot CLI session path ${copilotCliSessionPath}: ${checkError}`,
        );
      }
      ...
      const openCodeDataDir = this.getOpenCodeDataDir();
      const openCodeSessionDir = path.join(
        openCodeDataDir,
        "storage",
        "session",
      );
      const openCodeDbPath = path.join(openCodeDataDir, "opencode.db");
      this.log(
        `📁 Checking OpenCode JSON path: ${openCodeSessionDir} (exists: ${fs.existsSync(openCodeSessionDir)})`,
      );
      this.log(
        `📁 Checking OpenCode DB path: ${openCodeDbPath} (exists: ${fs.existsSync(openCodeDbPath)})`,
      );
      try {
        if (fs.existsSync(openCodeSessionDir)) {
          ...
          const openCodeCount =
            sessionFiles.length -
            sessionFiles.filter((f) => !this.isOpenCodeSessionFile(f)).length;
          if (openCodeCount > 0) {
            this.log(
              `📄 Found ${openCodeCount} session files in OpenCode storage`,
            );
          }
        }
      } catch (checkError) {
        ...
        if (fs.existsSync(openCodeDbPath)) {
          ...
          for (const sessionId of dbSessionIds) {
            if (!existingSessionIds.has(sessionId)) {
              ...
              sessionFiles.push(
                path.join(openCodeDataDir, `opencode.db#${sessionId}`),
              );
              dbNewCount++;
            }
          }
          if (dbNewCount > 0) {
            this.log(
              `📄 Found ${dbNewCount} additional session(s) in OpenCode database`,
            );
          }
        }
      } catch (dbError) {
   ...
   */
  private scanDirectoryForSessionFiles(
    dir: string,
    sessionFiles: string[],
  ): void {
    try {
  ...
  private isNonSessionFile(filename: string): boolean {
    ...
    return nonSessionFilePatterns.some((pattern) =>
      lowerFilename.includes(pattern),
    );
  }

  private async estimateTokensFromSession(
    sessionFilePath: string,
  ): Promise<{ tokens: number; thinkingTokens: number; actualTokens: number }> {
    try {
      ...
      const isJsonlContent =
        sessionFilePath.endsWith(".jsonl") || this.isJsonlContent(fileContent);
      if (isJsonlContent) {
          ...
          if (request.response && Array.isArray(request.response)) {
            for (const responseItem of request.response) {
              ...
              if (responseItem.kind === "thinking" && responseItem.value) {
                totalThinkingTokens += this.estimateTokensFromText(
                  responseItem.value,
                  this.getModelFromRequest(request),
                );
                continue;
              ...
              if (responseItem.value) {
                totalOutputTokens += this.estimateTokensFromText(
                  responseItem.value,
                  this.getModelFromRequest(request),
                );
              }
   ...
   */
  private estimateTokensFromJsonlSession(fileContent: string): {
    tokens: number;
    thinkingTokens: number;
    actualTokens: number;
  } {
        ...
        if (
          ...
          for (const responseItem of event.v) {
            ...
            if (responseItem.value) {
              ...
              responseItem.kind === "markdownContent" &&
              responseItem.content?.value
            ) {
   ...
   */
  private async getOpenCodeMessagesForSession(
    sessionFilePath: string,
  ): Promise<any[]> {
   ...
   */
  private async getTokensFromOpenCodeSession(
    sessionFilePath: string,
   ...
   */
  private async countOpenCodeInteractions(
    sessionFilePath: string,
  ): Promise<number> {
    const messages = await this.getOpenCodeMessagesForSession(sessionFilePath);
   ...
   */
  private async getOpenCodeModelUsage(
    sessionFilePath: string,
  ): Promise<ModelUsage> {
    const modelUsage: ModelUsage = {};
    const messages = await this.getOpenCodeMessagesForSession(sessionFilePath);
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const turnAssistantMsgs = messages.filter(
        (m, idx) => idx > i && m.role === "assistant" && m.parentID === msg.id,
      );
      ...
      const model =
        turnAssistantMsgs[0].modelID ||
        turnAssistantMsgs[0].model?.modelID ||
        "unknown";
      if (!modelUsage[model]) {
        modelUsage[model] = { inputTokens: 0, outputTokens: 0 };
      }
      // Output tokens are the sum of actual output+reasoning across the turn's API calls
      const turnOutput = turnAssistantMsgs.reduce(
        (sum, m) => sum + (m.tokens?.output || 0) + (m.tokens?.reasoning || 0),
        0,
      );
   ...
   */
  private async getOpenCodeSessionData(sessionFilePath: string): Promise<{
    tokens: number;
    interactions: number;
    modelUsage: ModelUsage & {
      [key: string]: {
  ...
  private getModelFromRequest(request: any): string {
    ...
    if (request.result && request.result.details) {
      ...
      const sortedDisplayNames = Object.keys(displayNameToModelId).sort(
  ...
  private estimateTokensFromText(
    ...
    for (const [modelKey, ratio] of Object.entries(this.tokenEstimators)) {
      if (
        model.includes(modelKey) ||
        model.includes(modelKey.replace("-", ""))
      ) {
        tokensPerChar = ratio;
        break;
      }
  ...
  public async showLogViewer(sessionFilePath: string): Promise<void> {
    ...
    // Set the HTML content
    this.logViewerPanel.webview.html = this.getLogViewerHtml(
      this.logViewerPanel.webview,
      logData,
    );
    ...
    this.logViewerPanel.webview.onDidReceiveMessage(async (message) => {
      switch (message.command) {
        case "openRawFile":
          try {
            ...
            vscode.window.showErrorMessage(
              "Could not open raw file: " + sessionFilePath,
            );
          }
          break;
        case "showToolCallPretty": {
          const { turnNumber, toolCallIdx } = message as {
            turnNumber: number;
            toolCallIdx: number;
          };
          this.log(
            `showToolCallPretty: turn=${turnNumber}, toolCallIdx=${toolCallIdx}, file=${sessionFilePath}`,
          );
          try {
            const turn = logData.turns.find((t) => t.turnNumber === turnNumber);
            const turnIndex = logData.turns.findIndex(
              (t) => t.turnNumber === turnNumber,
            );
            const toolCall = turn?.toolCalls?.[toolCallIdx];
            if (!toolCall) {
              this.log(
                "showToolCallPretty: tool call not found in session data",
              );
              vscode.window.showInformationMessage(
                "Tool call not found in session data.",
              );
              break;
            }
            ...
            const mapTurnForContext = (t?: ChatTurn) =>
              t
                ? {
            ...
            const mapToolCallForContext = (
              tc: { toolName: string; arguments?: string; result?: string },
              idx: number,
              parentTurn?: ChatTurn,
            ) => ({
            ...
            const sanitize = (name: string) =>
              name.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 60) || "toolcall";
            const prettyName = sanitize(
              `${toolCall.toolName || "tool"}-turn-${turnNumber}-call-${toolCallIdx}`,
            );

            const prettyPayload = {
              turnBefore:
                turnIndex > 0
                  ? mapTurnForContext(logData.turns[turnIndex - 1])
                  : undefined,
              toolCall: mapToolCallForContext(toolCall, toolCallIdx, turn),
              turnAfter:
                turnIndex >= 0 && turnIndex < logData.turns.length - 1
                  ? mapTurnForContext(logData.turns[turnIndex + 1])
                  : undefined,
            };
            ...
            const openDoc = vscode.workspace.textDocuments.find(
              (d) => d.uri.toString() === prettyUri.toString(),
            );
            if (openDoc) {
            ...
            vscode.window.showErrorMessage(
              "Could not open formatted tool call.",
            );
          }
          break;
        }
        case "revealToolCallSource": {
          ...
          this.log(
            `revealToolCallSource: turn=${turnNumber}, toolCallIdx=${toolCallIdx}, file=${sessionFilePath}`,
          );
          try {
            const turn = logData.turns.find((t) => t.turnNumber === turnNumber);
            const toolCall = turn?.toolCalls?.[toolCallIdx];
            if (!toolCall) {
              this.log(
                "revealToolCallSource: tool call not found in session data",
              );
              vscode.window.showInformationMessage(
                "Tool call not found in session data.",
              );
            ...
            this.log(
              `revealToolCallSource: searchTerm='${searchTerm}', matchIdx=${matchIdx}`,
            );

            const doc =
              await vscode.workspace.openTextDocument(sessionFilePath);
            const editor = await vscode.window.showTextDocument(doc);
            ...
            if (matchIdx >= 0) {
              ...
              vscode.window.showInformationMessage(
                "Opened session file, but could not locate this tool call text.",
              );
            }
          } catch (err) {
            this.error("revealToolCallSource: error", err);
            vscode.window.showErrorMessage(
              "Could not reveal tool call in file.",
            );
          }
          break;
        }
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        try {
          ...
          this.warn(
            `Skipping malformed line ${i + 1} in ${sessionFilePath}: ${e}`,
          );
        }
      }
  ...
  private getLogViewerHtml(
    webview: vscode.Webview,
    logData: SessionLogData,
  ): string {
    const nonce = this.getNonce();
    const scriptUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.extensionUri, "dist", "webview", "logviewer.js"),
    );
