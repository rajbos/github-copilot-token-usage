...
/**
 * Export Copilot Fluency Score as a multi-page PDF report
 */
private async exportFluencyScorePdf(maturityData: any): Promise<void> {
	try {
		...
		const addPageHeader = (pageNum: number, totalPages: number) => {
			...
			pdf.text(`Copilot Fluency Score Report - Page ${pageNum} of ${totalPages}`, margin, 10);
			pdf.text(new Date().toLocaleDateString(), pageWidth - margin, 10, { align: 'right' });
		};

		// PAGE 1: Cover page with spider chart
		...
		pdf.text('GitHub Copilot', pageWidth / 2, 30, { align: 'center' });
		pdf.text('Fluency Score Report', pageWidth / 2, 40, { align: 'center' });

		// Overall stage
		pdf.setFontSize(18);
		const overallLabel = maturityData.overallLabel || 'N/A';
		pdf.text(`Overall: ${overallLabel}`, pageWidth / 2, 55, { align: 'center' });

		// Spider chart description (simplified text representation)
		let yPos = 75;
		pdf.setFontSize(14);
		pdf.setTextColor(0, 0, 0);
		...
		for (const cat of categories) {
			const stageText = stageLabels[cat.stage] || `Stage ${cat.stage}`;
			pdf.text(`${cat.icon} ${cat.category}: ${stageText}`, margin + 5, yPos);
			yPos += 7;
		}
		...
		yPos += 7;
		pdf.setFontSize(10);
		pdf.text('Last 30 days of activity', margin + 5, yPos);
		yPos += 7;
		const lastUpdated = maturityData.lastUpdated ? new Date(maturityData.lastUpdated).toLocaleString() : 'N/A';
		pdf.text(`Last Updated: ${lastUpdated}`, margin + 5, yPos);

		// Footer
		pdf.setFontSize(8);
		pdf.setTextColor(128, 128, 128);
		pdf.text('Generated by Copilot Token Tracker Extension', pageWidth / 2, pageHeight - 10, { align: 'center' });

		// PAGE 2+: One page per category with details
		for (let i = 0; i < categories.length; i++) {
			pdf.addPage();
			const cat = categories[i];
			const pageNum = i + 2;

			addPageHeader(pageNum, totalPages);

			// Category title
			yPos = 25;
			pdf.setFontSize(18);
			pdf.setTextColor(0, 0, 0);
			pdf.text(`${cat.icon} ${cat.category}`, margin, yPos);

			// Stage badge
			yPos += 10;
			pdf.setFontSize(14);
			const currentStage = stageLabels[cat.stage] || `Stage ${cat.stage}`;
			pdf.text(currentStage, margin, yPos);
			...
			if (cat.evidence && cat.evidence.length > 0) {
				pdf.setFontSize(10);
				for (const evidence of cat.evidence) {
					const lines = pdf.splitTextToSize(`• ${evidence}`, contentWidth - 5);
					for (const line of lines) {
				...
				pdf.setFontSize(10);
				pdf.text('No evidence available', margin + 5, yPos);
				yPos += 7;
			}
			...
			pdf.setFontSize(12);
			pdf.setFont('helvetica', 'bold');
			pdf.text('Tips to Reach Next Stage:', margin, yPos);
			pdf.setFont('helvetica', 'normal');
			yPos += 7;
			...
			if (cat.tips && cat.tips.length > 0) {
				...
				pdf.setFontSize(10);
				pdf.text('You have reached the highest stage!', margin + 5, yPos);
				yPos += 7;
			}
