      ...
      Combobox = class extends FormAssociatedCombobox {
        ...
        set value(next) {
          ...
          if (this.$fastController.isConnected && this.options) {
            const selectedIndex = this.options.findIndex((el2) => el2.text.toLowerCase() === next.toLowerCase());
        ...
        setDefaultSelectedOption() {
          if (this.$fastController.isConnected && this.options) {
            const selectedIndex = this.options.findIndex((el2) => el2.getAttribute("selected") !== null || el2.selected);
            this.selectedIndex = selectedIndex;
      ...
      Picker = class extends FormAssociatedPicker {
        ...
        updateFilteredOptions() {
          ...
          if (this.filterSelected) {
            this.filteredOptionsList = this.filteredOptionsList.filter((el2) => this.selectedItems.indexOf(el2) === -1);
          }
          if (this.filterQuery && this.query !== "" && this.query !== void 0) {
            this.filteredOptionsList = this.filteredOptionsList.filter((el2) => el2.indexOf(this.query) !== -1);
          }
        }
        /**
      ...
      Menu = class _Menu extends FoundationElement {
        constructor() {
          ...
          this.setItems = () => {
            ...
            function elementIndent(el2) {
              const role = el2.getAttribute("role");
              const startSlot = el2.querySelector("[slot=start]");
          ...
          this.isMenuItemElement = (el2) => {
            return isHTMLElement(el2) && _Menu.focusableElementRoles.hasOwnProperty(el2.getAttribute("role"));
          };
          this.isFocusableElement = (el2) => {
            return this.isMenuItemElement(el2);
          };
        }
    ...
    "node_modules/@microsoft/fast-foundation/dist/esm/tree-view/tree-view.js"() {
      ...
      TreeView = class extends FoundationElement {
        constructor() {
          ...
          this.isFocusableElement = (el2) => {
            return isTreeItemElement(el2);
          };
          this.isSelectedElement = (el2) => {
            return el2.selected;
          };
        }
...
`;
  ...
  var initialData = window.__INITIAL_DETAILS__;
  ...
  function buildMetricsSection(stats, projections) {
    ...
    const headers = [
      { icon: "\u{1F4CA}", text: "Metric" },
      { icon: "\u{1F4C5}", text: "Today" },
      { icon: "\u{1F4C8}", text: "Last 30 Days" },
      { icon: "\u{1F4C6}", text: "Previous Month" },
      { icon: "\u{1F30D}", text: "Projected Year" }
    ];
    ...
    const rows = [
      { label: "Tokens (total)", icon: "\u{1F7E3}", color: "#c37bff", today: formatNumber(stats.today.tokens), last30Days: formatNumber(stats.last30Days.tokens), lastMonth: formatNumber(stats.lastMonth.tokens), projected: formatNumber(projections.projectedTokens) },
    ...
    rows.forEach((row) => {
      ...
      if (row.color) {
        iconSpan.style.color = row.color;
      }
  ...
  function buildEditorTbody(stats, allEditors) {
    ...
    const items = allEditors.map((editor) => {
    ...
    items.sort((a, b) => {
      ...
      switch (editorSortKey) {
        case "name":
          ...
          break;
        case "today":
          cmp = a.todayUsage.tokens - b.todayUsage.tokens;
          break;
        case "last30Days":
          cmp = a.last30DaysUsage.tokens - b.last30DaysUsage.tokens;
          break;
        case "lastMonth":
          cmp = a.lastMonthUsage.tokens - b.lastMonthUsage.tokens;
          break;
        case "projected":
          cmp = a.projectedTokens - b.projectedTokens;
          break;
        default:
          cmp = 0;
      }
    ...
    items.forEach(({ editor, todayUsage, last30DaysUsage, lastMonthUsage, projectedTokens, projectedSessions }) => {
      ...
      const todaySub = el("div", "muted", `${formatPercent(todayPercent)} \xB7 ${todayUsage.sessions} sessions`);
      todayTd.append(todaySub);
      const last30DaysTd = document.createElement("td");
      last30DaysTd.className = "value-right align-right";
      last30DaysTd.textContent = formatNumber(last30DaysUsage.tokens);
      const last30DaysSub = el("div", "muted", `${formatPercent(last30DaysPercent)} \xB7 ${last30DaysUsage.sessions} sessions`);
      last30DaysTd.append(last30DaysSub);
      const lastMonthTd = document.createElement("td");
      lastMonthTd.className = "value-right align-right";
      lastMonthTd.textContent = formatNumber(lastMonthUsage.tokens);
      const lastMonthSub = el("div", "muted", `${formatPercent(lastMonthPercent)} \xB7 ${lastMonthUsage.sessions} sessions`);
  ...
  function buildEditorUsageSection(stats) {
    ...
    const section = el("div", "section");
    const heading = el("h3");
    heading.textContent = "\u{1F4BB} Usage by Editor";
    section.append(heading);
    const table = document.createElement("table");
    table.className = "stats-table";
    const thead = document.createElement("thead");
  ...
  function buildModelTbody(stats, allModels) {
    ...
    items.sort((a, b) => {
      ...
      switch (modelSortKey) {
        case "name":
          ...
          break;
        case "today":
          cmp = a.todayTotal - b.todayTotal;
          break;
        case "last30Days":
          cmp = a.last30DaysTotal - b.last30DaysTotal;
          break;
        case "lastMonth":
          cmp = a.lastMonthTotal - b.lastMonthTotal;
          break;
        case "projected":
          cmp = a.projected - b.projected;
          break;
        default:
          cmp = 0;
      }
  ...
  function buildModelUsageSection(stats) {
    ...
    const section = el("div", "section");
    const heading = el("h3");
    heading.textContent = "\u{1F3AF} Model Usage (Tokens)";
    section.append(heading);
    const table = document.createElement("table");
    table.className = "stats-table";
    const thead = document.createElement("thead");
  ...
  async function bootstrap() {
    ...
    const { provideVSCodeDesignSystem: provideVSCodeDesignSystem2, vsCodeButton: vsCodeButton2, vsCodeBadge: vsCodeBadge2 } = await Promise.resolve().then(() => (init_dist3(), dist_exports));
    provideVSCodeDesignSystem2().register(vsCodeButton2(), vsCodeBadge2());
    if (initialData) {
